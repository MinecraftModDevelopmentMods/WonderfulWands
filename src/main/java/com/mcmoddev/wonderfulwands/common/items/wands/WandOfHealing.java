	...
	public void onPlayerStoppedUsing(ItemStack srcItemStack, World world, EntityLivingBase playerEntity,
		...
		if (target instanceof EntityLivingBase) {
			...
			if (health < maxHealth) {
				...
				e.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("regeneration"),
					60, 0));
				e.addPotionEffect(new PotionEffect(Potion.getPotionFromResourceLocation("glowing"),
					60, 0));
				if (playerEntity instanceof EntityPlayer && !((EntityPlayer)
